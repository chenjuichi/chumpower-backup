...
//export function useProcessTimer(timerRef) {
  ...
  function _startLocalTicker() {
	...
	*/
	...
	function _startLocalTicker(initialMs) {
		_stopLocalTicker()
		_baseMs = Number(initialMs || elapsedMs.value || 0)
		_lastTs = performance.now()

		const tick = (ts) => {
			// 若進來時已暫停，直接停掉
			if (isPaused.value) return
			const dt = Math.max(0, ts - _lastTs)
			_lastTs = ts
			elapsedMs.value = _baseMs + dt
			timerRef.value?.setState?.(elapsedMs.value, false)
  ...
  */
  ...
  function onTick(ms) {
	...
	*/
	// 假設後端回傳 { elapsed_ms, is_pause }
	async function restoreProcess(materialId, processType, userId, assembleId = 0) {
		const res = await apiRestore({ materialId, processType, userId, assembleId })
		const elapsed = Number(res?.elapsed_ms || 0)
		const paused  = !!res?.is_pause   // ← 後端欄位通常是 is_pause

		// 把狀態灌進 <TimerDisplay>
		timerRef.value?.setState?.(elapsed, paused)

		// ★ 關鍵：只有「未暫停」才啟動本地加時
		if (!paused) {
			_startLocalTicker(elapsed)
		} else {
			_stopLocalTicker()
		}
  ...
  async function toggleTimer() {
	...
	*/
	...
	async function apiPause(processId, elapsedMs) {
		// 1) 先把目前累積時間寫回（可選，但建議，避免重整後數字漂移）
		if (typeof elapsedMs === 'number') {
			try {
				...
				console.warn('update elapsed_ms before pause failed:', e)
			}
		}
		// 2) 真正暫停
		return dialog2ToggleProcess({
			process_id: processId,
			is_paused: true,
		})
	}

	async function apiResume(processId) {
		// 直接恢復（若你想在恢復當下也回寫一次 elapsed，可視需求加上 update）
		return dialog2ToggleProcess({
			process_id: processId,
			is_paused: false,
		})
	...
	async function toggleTimer() {
		if (!processId.value) return { success: false, message: 'NO_PROCESS' }

		if (isPaused.value) {
			// ← 目前是「暫停中」，按了要「恢復」
			const r = await apiResume(processId.value)
			if (r?.success) {
				isPaused.value = false
				_startLocalTicker()       // 你原本的本地計時啟動
				return { success: true }
			}
			return { success: false, message: r?.message || 'RESUME_FAIL' }
		} else {
			// ← 目前是「在跑」，按了要「暫停」
			const r = await apiPause(processId.value, elapsedMs.value)
			if (r?.success) {
				isPaused.value = true
				_stopLocalTicker()        // 你原本的本地計時停止
				return { success: true }
			}
			return { success: false, message: r?.message || 'PAUSE_FAIL' }
		}
	}
