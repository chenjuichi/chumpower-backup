...
if (!paused && data?.begin_time) {
  const beginMs = new Date(
    String(data.begin_time).replace(' ', 'T')
  ).getTime();

  if (!Number.isNaN(beginMs)) {
    const pauseSeconds = Number(data?.pause_time ?? 0);

    seconds = Math.max(
      0,
      Math.floor((Date.now() - beginMs) / 1000) - pauseSeconds
    );
  }
}
		//
		...
		*/
		if (paused) {
			...
			if (restoreOnly) {
				...
				//20260616 _startLocalTicker();
				_startAutoUpdate();
			} else {
				// 一般開始：照舊
				await nudgeResume();
				//20260616_startLocalTicker();
				_startAutoUpdate();
			}
		}
