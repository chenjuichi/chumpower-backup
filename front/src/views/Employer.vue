...
nextTick(() => {
  const role = empPermMapping[editedItem.emp_perm];
  const priv = roleMappings[role] || [];
  treeViewSelection.value = Array(26).fill(0).map((_, i) => (priv.includes(i + 1) ? 1 : 0));
  //treeViewSelection.value = [...editedItem.routingPriv]; // 使用深拷貝來觸發響應
});
...
//const role = empPermMapping[editedItem.emp_perm];
//const priv = roleMappings[role] || [];
//const role = roleMappings[empPermMapping[4]] || [];
//const priv = Array(26).fill(0).map((_, i) => (roleMappings['員工'].includes(i + 1) ? 1 : 0));
...
const editedItem = reactive({
  ...
  dep_name: '',
  emp_perm: 4,    // member
  routingPriv: Array(26).fill(0).map((_, i) => (roleMappings['員工'].includes(i + 1) ? 1 : 0)),
  password_reset: 'no',
});
...
const openDialog = () => {
  ...
  dialog.value = true;
  nextTick().then(() => {
    treeViewSelection.value = initialSelection;
  });

  //  treeViewSelection.value = Array(26).fill(0).map((_, i) => (roleMappings['員工'].includes(i + 1) ? 1 : 0));
  //})
};
...
const save = () => {
  ...
  editedItem.routingPriv = updateSelection(treeViewSelection.value);
  console.log("treeViewSelection:", treeViewSelection.value);
  console.log("editedItem:", editedItem, editedItem.routingPriv);
...
const updateSelection = (newSelection) => {
  ...
  //if (editedIndex.value != -1 && !showExplore.value) {
  //  showExplore.value = true
  //  //treeViewSelection.value = editItem.routingPriv;
  //}
...
const updateEmpPerm = (newValue) => {
  console.log("updateEmpPerm,", newValue);
  //editedItem.emp_perm = reverseEmpPermMapping[newValue];
};
</script>
