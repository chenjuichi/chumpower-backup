...
watch(switchValue, (val) => {
  ...
  switchValue_string.value =
    "只顯示未完成訂單編號"

  // 不在這裡呼叫 listInformations()
  //
  // 因為下面已經有：
  //
  // watch([
...
const runQuery = async () => {
  const payload =
    buildListInformationsPayload()

  const response =
    await listInformations(payload)

  // request 被新 request 取代時，
  // crud.js 會回傳 undefined
  if (!response) {
    return
  }
...
*/
// 20260830版
const applyListInformationsResponse = (
  response
) => {
  if (!response) {
    console.log(
  ...
  statusIds.not_prepare =
    response.status_ids?.not_prepare || []

  statusIds.prepare =
    response.status_ids?.prepare || []

  statusIds.assemble =
    response.status_ids?.assemble || []

  statusIds.warehouse =
    response.status_ids?.warehouse || []

  statusIds.stockin =
    response.status_ids?.stockin || []
}
//
