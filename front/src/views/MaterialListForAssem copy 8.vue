...
watch(group1, async (newVal, oldVal) => {
  ...
  // 20260901
  // 已經送出 / 已經產生 child 後，
  // 不允許再修改 parent 的 merge_enabled。
  //
  // merge_enabled 是這次缺料拆批的歷史決策，
  // 不是可以之後任意切換的顯示設定。
  // ============================================================

  if (
    editedRecord.value.isShow === true
    || editedRecord.value.is_copied_from_id
  ) {
    console.log(
      '[merge_enabled] skip update:',
      {
        id: editedRecord.value.id,
        isShow: editedRecord.value.isShow,
        is_copied_from_id:
          editedRecord.value.is_copied_from_id,
        current_merge_enabled:
      ...
      for (const id of selectedIds) {
        ...
        console.log(
          '[Material SEND]',
          {
            id: m.id,
            order_num: m.order_num,
            is_copied: m.is_copied,
            merge_enabled: m.merge_enabled,
            isShow: m.isShow,
            show2_ok: m.show2_ok,
          }
        )
...
const updateItem = async () => {    //編輯 bom, material及process後端table資料
  ...
  if (!take_out) {
    ...
    console.log(
      '[CREATE CHILD]',
      {
        source_id: editedRecord.value.id,
        order_num: editedRecord.value.order_num,
        source_merge_enabled: editedRecord.value.merge_enabled,
        group1: group1.value,
        calculated_merge_enabled: sourceMergeEnabled,
      }
    );
