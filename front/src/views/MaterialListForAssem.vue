<template>
  ...
  import { materials, boms, currentAGV, material_copy_id ,socket_server_ip, fileCount }  from '../mixins/crud.js';
  ...
  watch(boms,
    (newBoms) => {
      newBoms.forEach((bom_item) => {
        console.log(`bom_item ${bom_item.seq_num} receive status: ${bom_item.receive}`);

      });
    },
    { deep: true } // 設定為 deep 以便監控到物件屬性變化
  );
  */
  /*
  watch(boms, async (newBoms) => {
      // 如果任一項 bom_item.receive 為 false，則進行更新
      const hasFalseReceive = newBoms.some((bom_item) => !bom_item.receive);
          // 使用 .some() 檢查是否有任何 `receive` 為 false 的項目，若有則將 `take_out` 設為 false
    let     hasFalseReceive = !boms.value.some(bom => !bom.receive);
    console.log("take_out:", take_out);
      let payload = {};
      if (hasFalseReceive) {
        console.log(`id ${selectedItem.value.id} 缺料`);
        payload = {
        ...
        console.log(`id ${selectedItem.value.id} 沒有缺料`);
        payload = {
          id: selectedItem.value.id,
          record_name: 'shortage_note',
          record_data: '',
  ...
  */
  ...
  // 監聽 selectedItems 的變化，並將其儲存到 localStorage
    ...
    try {
      ...
      */
      socket.value.on('agv_start', async () => {
        console.log('AGV Start, 收到 agv_start 訊息');
      ...
      socket.value.on('station2_agv_end', async () => {
        ...
        selectedItems.value.forEach(async (item) => {
          ...
          ///let currentOrderNum = myMaterial.order_num;
          ///let sameOrderNumMaterials = materials.value.filter(m => m.order_num === currentOrderNum && m.id !== item);

          //if (sameOrderNumMaterials.length === 0) {
          //  console.log("1. 沒有其他資料有相同的 order_num...")
          //  // 1. 沒有其他資料有相同的 order_num
          ...
          //    material_num: myMaterial.material_num,
          //    material_comment: myMaterial.comment,
          //    material_qty: myMaterial.material_qty,
          //    total_delivery_qty: myMaterial.material_qty - myMaterial.delivery_qty,
          //    material_date: myMaterial.date,
          //    material_delivery_date: myMaterial.delivery_date,
          //  });
          //  */
          //} else {
          //console.log("myMaterial.delivery_qty, myMaterial.total_delivery_qty:", myMaterial.delivery_qty, myMaterial.total_delivery_qty)
          //if (sameOrderNumMaterials.length != 0) {
          if (Number(myMaterial.delivery_qty) != Number(myMaterial.total_delivery_qty)) { // 1張工單多批次運送
              console.log("1張工單多批次運送, 新增未運送數量(相同工單)")
            // 2. 如果有相同 order_num 並且所有 delivery_qty 都不為 0
            ///const totalDeliveryQty = sameOrderNumMaterials.reduce((sum, mat) => sum + Number(mat.delivery_qty), 0);
            ///const allNonZero = sameOrderNumMaterials.every(mat => Number(mat.delivery_qty) !== 0);
            ///console.log("sameOrderNumMaterials:",sameOrderNumMaterials);
            ///console.log("totalDeliveryQty:",totalDeliveryQty);
            ///console.log("allNonZero:",allNonZero);
            ///console.log("myMaterial.total_delivery_qty:",myMaterial.total_delivery_qty);
            ///console.log("myMaterial.delivery_qty:",myMaterial.delivery_qty);
            ...
            ///if (allNonZero) {
              ...
              let tempDelivery = myMaterial.total_delivery_qty - myMaterial.delivery_qty;
              ///else
              ///  tempDelivery = myMaterial.total_delivery_qty - totalDeliveryQty;
              ...
              /*
              materials.value.push({
                id: generateUniqueId(), // 需要一個函數來生成唯一的 ID
                order_num: currentOrderNum,
                material_qty: myMaterial.material_qty,
                delivery_qty: 0,
                total_delivery_qty: tempDelivery
      ...
      socket.value.on('station1_agv_ready', async () => {
        console.log('AGV 已到達備料區裝卸站, 收到 station1_agv_ready 訊息...');
        // 記錄等待agv到站結束時間
  ...
  */
  ...
  const updateItem2 = async (item) => {
    ...
    //inputValueForItems.value = materials.value.map(material => material.delivery_qty);
    //localStorage.setItem('inputValueForItems', JSON.stringify(inputValueForItems.value));
    ...
    //};
    //await updateMaterial(payload);
    //item.total_delivery_qty = deliveryQty;
    /*
    let temp1 = Number(item.total_delivery_qty)
    let temp2 = deliveryQty
    // 確認 已送料總數量=需求數量
    if (temp2 == temp1) {
      // 記錄當前紀錄, 不能再輸入
      payload = {
  ...
  const updateItem = async () => {              //編輯 bom, material及process後端table資料
    ...
    let materialPayload = {}
    /*
    materialPayload = {             // 更新 materials 資料中, 已備料總數
      id: selectedItem.value.id,
      record_name: 'delivery_qty',
