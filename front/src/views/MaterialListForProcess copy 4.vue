  ...
  try {
    ...
    */
    ...
    if (socket?.value) {
      ...
      console.log(
        '[PMaterial] process-material-refresh listener registered'
      )
    }
...
onBeforeUnmount(() => {
  ...
  */
  ...
  if (socket?.value) {
    ...
    try { socket.value.off?.('process-material-refresh', onProcessMaterialRefresh); } catch (_) {}
...
// 20260824版
const onProcessMaterialRefresh = async (data) => {
  ...
  try {
    ...
    // 20260824
    // PEnd 前一道加工送出後，
    // PMaterial 自動重新抓 /listMaterialsP
    //
    // 例如：
    //
    // 作業50完成並送出
    //      ↓
    // 收到 process-material-refresh
    //      ↓
    // fetchMaterials()
    //      ↓
    // /listMaterialsP
    //      ↓
    // 作業60通過 previous_process_handoff_done()
    //      ↓
    // PMaterial立即顯示
    // ========================================================
    ...
    console.log(
      '[process-material-refresh] refresh completed',
      data?.order_num || ''
    )
    ...
    console.error(
      '[process-material-refresh] refresh failed:',
      error
    )
  }
}
