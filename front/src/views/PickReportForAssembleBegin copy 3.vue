              ...
              <div class="flip_btn fixed-flip-btn">
                ...
                <div class="side hover-side hover-actions">
                  ...
                  <v-btn
                    class="action-btn"
                    :disabled="isInformationEmpty"
                    @click="!isInformationEmpty && batchRemoveRecords()"
                  >
...
<!--  :style="{ opacity: (currentUser.perm == 1 || currentUser.perm == 2)  ? 1 : 0, visibility: (currentUser.perm == 1 || currentUser.perm == 2) ? 'visible' : 'hidden' }"-->
  ...
  -->
    ...
    <template #item.action="{ item }">
      <!-- 開始鍵左側顯示「自己」的計時值 -->
      <!--<span v-if="isMineStarted(item) || item.show_name == userId" -->
      <!--<span v-if="item._showMyTimer || item.show_name == userId"-->
      <!--<span v-if="isMineStarted(item) || item.show_name == userId"-->
      <span v-if="item._showMyTimer || isMineStarted(item) || item.show_name == userId"
      ...
      <!-- :style="item.show_name === userId ? { position: 'relative', left: '70px' } : {position: 'relative', left: '117px'}" -->
        ...
        <v-btn
          ...
          style="font-size:14px; font-weight:700; font-family: '微軟正黑體', sans-serif; position:relative; left:70px;"
...
async function onClickBegin(row) {
  ...
  row._showMyTimer = true;
  row.show_timer = true;
  row.show_name = safeUserId.value;
