...
const filteredInformations = computed(() => {
  return informations_for_assemble_error.value
    .map(item => ({
      ...
      // 20260908
      // 每一筆 Error 都使用自己的 cause_message
      // 不可用 order_num 共用，因為同訂單可能有多筆 assemble
      cause_message: item.cause_message ?? '',
    }))
    .filter(item => {
      const isWithinDateRange =
        checkDateInRange(item.delivery_date);
...
async function editCauseMessage(item) {
  console.log(
    'editCauseMessage(), item:',
    item
  )

  // 20260908
  // 同一 order_num 可能同時有多筆 Error，
  // 必須用 assemble_id 找到真正被編輯的那一筆
  editedIndex.value =
    filteredInformations.value.findIndex(
      row =>
        Number(row.assemble_id) ===
        Number(item.assemble_id)
    )

  console.log(
    '[editCauseMessage] assemble_id:',
    item.assemble_id,
    'editedIndex:',
    editedIndex.value
  )
...
// 20260908版
// 20260908版
async function confirmCauseMsg () {
  if (!causeDlg.target) return

  const finalMsg =
    String(causeDlg.form.msg || '').trim()

  if (!finalMsg) return

  const assembleId =
    Number(causeDlg.target.assemble_id)

  console.log(
    '[confirmCauseMsg] assemble_id:',
    assembleId,
    'cause_message:',
    finalMsg
  )
  ...
  // 先送後端
  // --------------------------------------------------
  const success = await onValueUpdate({
    ...causeDlg.target,
    assemble_id: assembleId,
    cause_message: finalMsg,
  })
  ...
  // 20260908
  // 重要：
  // filteredInformations 是 computed copy，
  // 所以必須修改真正的原始資料
  // --------------------------------------------------
  const sourceRow =
    informations_for_assemble_error.value.find(
      row =>
        Number(row.assemble_id) === assembleId
    )

  if (sourceRow) {
    sourceRow.cause_message = finalMsg
  }
...
const onValueUpdate = async (item) => {
  ...
  const payload = {
    assemble_id: Number(item.assemble_id),
    cause_message:
      String(item.cause_message || '').trim(),
    cause_user:
      currentUser.value?.empID || '',
  }

  console.log(
    '[updateAssembleAlarmMessage] payload:',
    payload
  )
  ...
  try {
    ...
    console.log(
      '[updateAssembleAlarmMessage] 更新成功',
      payload
    )
    ...
    console.error(
      '[updateAssembleAlarmMessage] 更新失敗:',
      error
    )
...
/* 20260908版
   不要全域修改 v-input__control，
   否則會影響 data-table Footer 的每頁筆數選擇框
*/
...
/* 20260908版：Footer 每頁資料筆數，比照 End */
