<template>
  ...
  />
        ...
        <template v-slot:item.action="{ item }">
          ...
          -->
              <TimerDisplay
  :key="`${makeKey(item)}:${item.process_id || 0}:${item.process_elapsed_time || 0}`"
...
function closedDisplayMs(item) {
  ...
  const secs = Number(
    item.process_elapsed_time ??
    item.elapsedActive_time ??
    item.elapsed_time ??
    0
  )
...
async function ensureRestored(row, force = false) {
  ...
  // ✅ 已結束 / 待送出列，只顯示固定時間，不 restoreProcess
  if (isRowClosed(row)) {
    const ms = closedDisplayMs(row)
    if (ms != null) {
      frozenMsMap.set(k, ms)
    }
    return getT(row)
