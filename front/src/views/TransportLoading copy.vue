...
import forkliftSvg from "../assets/icon-forklift.svg"
import agvSvg from "../assets/icon-forklift.svg"
...
const props = defineProps({
  ...
  showText: { type: Boolean, default: true },
	...
	durationSec: { type: Number, default: 4 },
})
...
const vehicleSrc = computed(() => {
  return props.mode === 'agv'
    ? agvSvg        // 若沒有 AGV svg，可先 return forkliftSvg
    : forkliftSvg
})
...
const wrapStyle = computed(() => ({
  ...
  '--track-bottom': `${trackBottom.value}px`,
	...
	'--duration': `${Math.max(0.2, Number(props.durationSec) || 4)}s`,
}))
