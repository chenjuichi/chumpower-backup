...
<!--20260807版-->
<v-text-field
  v-model.number="item.allOk_qty"
  type="number"
  min="0"
  :max="Number(item.must_allOk_qty || 0)"
  :disabled="item.input_allOk_disable || (item.is_lack_batch_order && !item.all_batches_waiting)"
  :hint="item.is_lack_batch_order ? item.waiting_message : ''"
  persistent-hint
  @update:modelValue="value => checkStockinQty(item, value)"
/>
        <span
...
const checkStockinQty = (item, value) => {
  const qty = Number(value || 0)
  const remainQty = Number(
    item.must_allOk_qty || 0
  )

  if (
    item.is_lack_batch_order
    && !item.all_batches_waiting
  ) {
    item.allOk_qty = 0
    return false
  }
...
const buildCreateProductPayload = (item) => {
  if (
    item.is_lack_batch_order
    && item.is_order_merged
  ) {
    if (!item.all_batches_waiting) {
      throw new Error(
        '缺料分批尚未全部到庫，不可入庫'
      )
    }
    ...
    const requiredQty = targets.reduce(
      (sum, target) => {
        return (
          sum
          + Number(
    ...
    return {
      items: targets.map(target => ({
        material_id:
          Number(target.material_id),

        assemble_id:
          Number(target.assemble_id),

        process_id:
          Number(target.process_id || 0),

        delivery_qty:
          Number(target.delivery_qty || 0),
