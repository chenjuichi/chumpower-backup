    ...
    for (const id of selectedIds) {
      ...
      */
      ...
      const productPayload = {
        ...
        // 組裝線 Warehouse 上的 process_id 通常是搬運 3 / 6，
        // 入庫 process 31 交給後端建立。
        process_id:
          current_line === 'assemble'
            ? 0
            : current_process_id,
        ...
        assemble_qty:
          current_line === 'assemble'
            ? 0
            : Number(row.assemble_qty || 0),

        // ----------------------------------------------------------
        // 20260909
        // 組裝線 order-level 入庫資訊
        // 後端仍會依 order_num 自己重新取得完整 material group，
        // 這裡主要提供資訊與相容性。
        // ----------------------------------------------------------
        ...
        is_order_level_warehouse:
          row.is_order_level_warehouse === true,
