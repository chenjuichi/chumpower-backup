...
const headers = [
  ...
  { title: '說明', align: 'start', sortable: false, key: 'comment', width:320 },  // 2025-06-13 modify, 改順序
...
const qtyResetTimers = new Map();  // key: item.id, value: timer handle
...
const checkQtyField = (item) => {
  // 轉數字；避免 '' / null 之類造成比較錯誤
  const mustQty  = Number(item.must_allOk_qty) || 0;   // 應入庫數量
  const inputQty = Number(item.allOk_qty)    || 0;     // 入庫數量（當前輸入）

  // 先清掉舊的計時器（很關鍵：避免舊的 timeout 晚點把值清 0）
  const old = qtyResetTimers.get(item.id);
  if (old) {
    clearTimeout(old);
    qtyResetTimers.delete(item.id);
  }

  // 超量 → 顯示提示 & 設一個「仍超量才清 0」的保護性計時器
  if (inputQty > mustQty) {
    ...
    const handle = setTimeout(() => {
      // 2 秒後再檢查一次當前值是否「仍然」超量；若已修正就不動
      const nowQty = Number(item.allOk_qty) || 0;
      if (nowQty > (Number(item.must_allOk_qty) || 0)) {
        item.allOk_qty = 0;
      }
    ...
    qtyResetTimers.set(item.id, handle);
  } else {
    // 合法 → 關閉提示、清除錯誤狀態（顏色改由樣式判斷）
    item.tooltipVisible = false;
    item.isError = false;
  }
};
