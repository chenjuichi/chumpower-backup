...
const onClickWarehouseIn = async () => {
  ...
  try {
    ...
    for (const id of selectedIds) {
      ...
      console.log("[WAREHOUSE] stockin qty d2 =", d2, "order=", row.order_num, "material_id=", current_material_id, "assemble_id=", current_assemble_id)
      ...
      console.log("[WAREHOUSE] productPayload =", productPayload)
      ...
      const productPayload = {
        material_id: current_material_id,
        assemble_id: current_assemble_id,                 // ✅ 很重要（後端要標記 isStockIn）
        user_id: currentUser.value?.empID ?? '',          // ✅ 讓後端寫入 process/user
        line_difference: current_line === 'process' ? 1 : 0,
        process_id: procResp?.process_id,
        allOk_qty: d2,
        good_qty: d2,                                     // ✅ 沒另輸入良品數，就用入庫量
        non_good_qty: 0,
