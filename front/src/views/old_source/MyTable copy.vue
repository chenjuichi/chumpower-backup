<template>
  <div class="page_contain">
    ...
    >
      ...
      <template v-slot:item.deptEname = "{ item }">
        <v-text-field
          ...
          @click:append-inner="onClickInnerIcon(item)"
    ...
    <v-dialog v-model="combobox_dialog" persistent max-width="290">
      <v-card>
      <v-card-title class="headline">部門名稱</v-card-title>
      <v-card-text>
        <v-combobox
          v-model="selectedEname"
...
<script>
import { ref, watch, onMounted, onBeforeMount, computed, nextTick, onBeforeUnmount, } from 'vue';
...
export default {
  ...
  setup() {
    ...
    const combobox_currentItem = ref(null);
    const selectedEname = ref(null);
    const itemToDelete = ref(null);
    const currentItem = ref(null);  //for combobox
    ...
    const onClickInnerIcon = (item) => {
      console.log("onClickInnerIcon(), item", item);

      if (item && item.deptEname !== undefined) {
        currentItem.value = { ...item };
        selectedEname.value = item.deptEname; // 设置当前选择的值
        combobox_dialog.value = true;
      } else {
        console.error("onClickInnerIcon(): item 数据不完整或未定义", item);
      }
    };

    const onComboboxChange = (value) => {
      console.log("onComboboxChange(), value, ", value);

      if (currentItem.value) {
        currentItem.value.deptEname = value;
      }
    };

    const saveComboboxValue = () => {
      if (currentItem.value) {
        const index = desserts.value.findIndex(item => item.id === currentItem.value.id);
        if (index !== -1) {
          desserts.value[index] = { ...currentItem.value };
        }
    ...
    const clearMessage = () => {
      if (editedItem.value && editedItem.value.deptId !== undefined) {
        ...
        checkForDuplicates(''); // 传递空字符串而不是 null
      }
    };
    ...
    const setRowClass = (data) => {
      ...
      if (data.item && data.item.deptId) {
        const temp1 = data.item.deptId.trim().length;
        //console.log('deptId:', data.item.deptId);
        //console.log('temp1:', temp1);
        const temp2 = data.item.deptCname.trim().length;

        if (last_counter.value-1 == data.item.id) {
          //const temp1=data.item.deptId.trim().length
          //const temp2=data.item.deptCname.trim().length
          if (temp1 == 0 || temp2 == 0) {
            isSaveOK.value = true;
    ...
    const checkForDuplicates = (deptId) => {
      ...
      if (deptId.length >= 3) {
        const matchingItems = desserts.value.filter(item => item.deptId === deptId && item.id != last_counter.value-1);
    ...
    watch(list_table_is_ok, (val) => {
      if (val) {
        //const newArray = temp_desserts.value.map(v => ({ ...v, id: counter.value++, isReadonly: false }));
        //desserts.value = Object.assign([], newArray);

        desserts.value = [
          { id: 1, isReadonly: false, deptId: '100', deptCname: '會計100', deptEname: 'FIN_E100', deptAname: 'FIN_A100' },
          { id: 2, isReadonly: false, deptId: '110', deptCname: '會計110', deptEname: 'FIN_E110', deptAname: 'FIN_A110' },
          { id: 3, isReadonly: false, deptId: '120', deptCname: '會計120', deptEname: 'FIN_E120', deptAname: 'FIN_A120' },
          { id: 4, isReadonly: false, deptId: '130', deptCname: '會計130', deptEname: 'FIN_E130', deptAname: 'FIN_A130' },
          { id: 5, isReadonly: false, deptId: '140', deptCname: '會計140', deptEname: 'FIN_E140', deptAname: 'FIN_A140' },
          { id: 6, isReadonly: false, deptId: '150', deptCname: '會計150', deptEname: 'FIN_E150', deptAname: 'FIN_A150' },
          { id: 7, isReadonly: false, deptId: '160', deptCname: '會計160', deptEname: 'FIN_E160', deptAname: 'FIN_A160' },
          { id: 8, isReadonly: false, deptId: '170', deptCname: '會計170', deptEname: 'FIN_E170', deptAname: 'FIN_A170' }
        ];
        ...
        console.log(combobox_desserts.value)
        //const combobox_ary_desserts = ref(combobox_desserts.value.map(item => item.deptEname));
        //const combobox_ary_desserts = ref(combobox_desserts.value.map(item => item.deptEname));
        const combobox_ary_desserts = ref(combobox_desserts.value.map(item => item.deptEname));

        console.log(combobox_ary_desserts.value)
        console.log("...combobox example...")
