...
def process_image_with_text_only(input_path, output_path, text_data):
  with Image.open(input_path) as img:
    ...
    # 插入姓名與日期
    font1 = ImageFont.truetype(font_path, 24)
    draw.text((28, 4), text_data.get("first_name", ""), fill="blue", font=font1)

    font2 = ImageFont.truetype(font_path, 15)
    draw.text((4, 30), text_data.get("stamp_date", ""), fill="blue", font=font2)

    font3 = ImageFont.truetype(font_path, 24)
    draw.text((16, 46), text_data.get("last_name", ""), fill="blue", font=font3)
...
def save_file():
  ...
  try:
    ...
    # 回傳 JSON 而非直接下載
    return jsonify({
      'message': 'PDF saved with barcode',
...
def stamp_file():
  ...
  try:
    ...
    # 加入白色遮罩 (Optional 覆蓋版)
    stamped_path_final = os.path.join(base_directory, f"NEW_{original_filename}")
    add_white_mask_to_pdf(stamped_path, stamped_path_final)

    return jsonify({
      'message': 'PDF stamped with image and date',
      'filename': f"NEW_{original_filename}",  # 此檔案將會是最終下載用
      'filepath': stamped_path_final
    })
