    ...
    try:
        ...
        if process_type_int in (2, 5):
            ...
            if existed_inbound_transport is not None:
                ...
                # 20260827
                # 雖然搬運 process 已存在，
                # 仍必須確保 material / assemble 已釋放到 Begin。
                #
                # 防止：
                # process 已建立，
                # 但 material 狀態尚未同步完成，
                # 後續重送又因 duplicate 直接 return，
                # 導致資料永久留在 Material。
                # --------------------------------------------------------
                release_to_assemble_begin(
                    session=s,
                ...
                print(
                    "[createProcess] inbound transport exists, "
        ...
        if process_type_int == 19:
            ...
            # 代表已選擇手動/堆高機，不應再建立等待 AGV。
            # ============================================================

            if material.move_by_automatic_or_manual is False:

                s.rollback()

                print(
                    "[createProcess] type19 skipped, "
                    "material uses forklift:",
                    material_id_int
                )
...
def copy_material_and_bom():
    ...
    try:
        ...
        #
        # 即使後續缺料 BOM 被搬走、補齊，
        # parent.shortage_note 仍必須保留 "(缺料)"，
        # 讓 Begin 顯示：
        #
        #     訂單號碼 + 缺料不併單
        #
        # 注意：
        # shortage_note 在這裡是歷史標記，
        # 不能拿它直接判定目前還缺料。
        # ============================================================
        '''
        if not _merge_enabled:

            existing_material.shortage_note = "(缺料)"

            existing_material.merge_enabled = False
        '''
        #
        # ============================================================
        # 20260826
        # 缺料併單 / 不併單設定必須同步到來源 material
        #
        # merge_enabled=True
        #     parent / copy 都是併單
        #
        ...
        existing_material.merge_enabled = bool(
            _merge_enabled
        )
        ...
        # ============================================================
        # 20260908
        # 只要有 missing_boms 並建立缺料 copy，
        # 就代表來源 parent 這一批確實發生過缺料。
        #
        # Begin / Material 顯示「缺料」會依 isLackMaterial 判斷，
        # 所以 parent 必須保留缺料狀態。
        # ============================================================

        existing_material.isLackMaterial = 1
        #
...
def copy_material():
    ...
    try:
        ...
        order_num = str(
            existing_material.order_num
            or ""
        ).strip()

        # ====================================================
        # 20260908
        # 1-1. 找這張訂單的 root Material
        #
        # root 的 material_qty 視為原始訂單需求數量。
        #
        # 例如：
        #
        #   material 171
        #   material_qty = 39
        #
        # 後面的 186 / 572 都是 child，
        # 不可以拿 child 的 material_qty 當訂單總需求。
        # ====================================================
        root_material = (
            s.query(Material)
            .filter(
        ...
        if root_material:
            order_required_qty = int(
                root_material.material_qty
                or 0
            )
        else:
            order_required_qty = int(
                existing_material.material_qty
                or 0
            )
        ...
        # 只看 process_type = 31。
        #
        # 同一 material 若意外有多筆 type=31，
        # 只取最大一筆，避免重複累加。
        # ====================================================
        stockin_by_material = (
            s.query(
                Process.material_id.label(
                    "material_id"
                ),
        ...
        #   stockin_total = 39
        #
        # 所以今天再呼叫 copyMaterial 時，
        # 必須在這裡直接擋掉，
        # 不可再建立 material 572 / assemble 1573。
        # ====================================================
        if (
        ...
        if (
            ...
            return jsonify({
                ...
                "message":
                    (
                        "剩餘數量異常："
                        f"{_total_delivery_qty} "
                        f"不可大於訂單數量 "
                        f"{order_required_qty}"
                    ),
            }), 200
            ...
            # ====================================================
                ...
                for assemble_row in (
                    ...
                    if (
                        ...
                        # 群組入庫：
                        #
                        # shortage / order-level 都要同步關閉 backing rows
                        #
                        # 注意：
                        # 不要把整筆 add_qty 都寫進每一個 assemble。
                        #
                        # 1087 completed=15
                        # 1090 completed=5
                        #
        ...
        for row in normalized_items:
            ...
            if not material_finished:
                ...
                if (
                    ...
                    #row_old_stockin = _normalize_int(
                    #    assemble_record.allOk_qty,
                    #    0
                    #)
                    #
                    ...
                    # 20260831 修正
                    #
                    # assemble.allOk_qty 是完工/良品數量，
                    # 不能當成已入庫數量。
                    #
                    # 已入庫數量必須以 process_type=31 為準，
                    # 且限定同一 material_id + assemble_id。
                    # ------------------------------------------------------------
                    row_old_stockin = (
                        s.query(
                            func.coalesce(
