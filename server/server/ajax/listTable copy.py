...
"""
def map_pt(row):
    return 21 if row.process_step_code == 3 else 22 if row.process_step_code == 2 else 23
"""
...
def active_count_map_by_material_multi(session, material_ids, process_types=(21,22,23), include_paused=True):
    ...
    if not include_paused:
        q = q.filter(or_(Process.is_paused.is_(False), Process.is_paused.is_(None)))
...
def _active_count_map_by_material(session, material_ids, process_type=21):
    """回傳 { material_id: 未結束流程數 }，用於「這筆 material 是否有人開工」的綠點判斷。"""
    if not material_ids:
        return {}
    rows = (
        session.query(Process.material_id, func.count(Process.id))
        .filter(Process.material_id.in_(material_ids))
        .filter(Process.process_type == process_type)  # 21=組裝（依你的系統）
        .filter(Process.end_time.is_(None))            # 未結束 = 正在開工
        .filter(Process.is_paused.is_(False))           # 2025-09-25 add
        .group_by(Process.material_id)
        .all()
    )
    return {mid: int(cnt) for (mid, cnt) in rows}

def _format_show2_ok(token_str, num, completed_qty, total_ask_qty_end):
    """
    把 '00/00/00' 顯示字串依規則替換。
    - num in [5,7,9] 時，將第 total_ask_qty_end 個 '00' 替換為 completed_qty
    """
    if num in (5, 7, 9):
        try:
            parts = (token_str or '').split('/')
            idx = int(total_ask_qty_end or 0) - 1
            if 0 <= idx < len(parts):
                parts[idx] = str(completed_qty)
            return '/'.join(parts)
        except Exception:
            return token_str or ''
    return token_str or ''

def _step_enable(material_record, assemble_record, max_step_code_per_order):
    """
    是否允許輸入/顯示此列：
    - 該 material 的最大 step_code 與此列相同
    - 且 whichStation == 2
    """
    order_num_id = material_record.id
    step_code = assemble_record.process_step_code
    max_step = max_step_code_per_order.get(order_num_id, 0)
    return (step_code == max_step and material_record.whichStation == 2)
...
def list_materials_and_assembles():
    ...
    for material_record in _objects:
      for assemble_record in material_record._assemble:
        if assemble_record.input_disable:
           continue
        #code = assemble_record.work_num[1:]  # 取得字串中的代碼 (去掉字串中的第一個字元)
        step_code = assemble_record.process_step_code  # 直接使用資料中的 step_code
    ...
    #  print("temp_temp_show2_ok_str:", temp_temp_show2_ok_str)
      ...
      for assemble_record in material_record._assemble:     # loop_am
        ...
        step_enable = (step_code == max_step_code and material_record.whichStation==2)
        print(assemble_record.id,":",step_enable, step_code, max_step_code, assemble_record.input_disable)
        ...
        _object = {
          ...
          'receive_qty': assemble_record.must_receive_qty,                # ready modify 2025-09-15
          #'receive_qty': assemble_record.ask_qty,                         # 領取數量
          ...
          # 有人正在這筆（material）開工的人數
          #'active_user_count': counts_map.get(material_record.id, 0),
          #'active_user_count': _pick_active_count(assemble_record.process_step_code, material_record.id),
        }
        #print(_object)
        _results.append(_object)
    ...
    # 你可以視需求塞回每一列，例如決定用當列的 step 對應的製程去取值：
    #def map_pt(row):
    #    return 21 if row.process_step_code == 3 else 22 if row.process_step_code == 2 else 23

    '''
    for row in _results:  # 你原本組前端資料的 loop
        #pt = str(map_pt(row))
        #mid = str(row.id)
        mid = str(get_val(row, 'id'))
        pt  = str(map_pt(row))
        row.active_user_count = counts_by_type.get(pt, {}).get(mid, 0)
        # 如果前端會用到整包 map（例如顯示不同製程的人數），也可回傳 counts_by_type
    '''
    for row in _results:  # rows 可能來自 ORM 或你 earlier 轉成 dict 的結果
      try:
          pt  = str(map_pt(row))            # 永遠回 21/22/23（字串）
          mid = str(get_val(row, 'id'))     # material_id
          # 這裡寫你需要的欄位，全部用 get_val 避免再噴
          row.active_user_count = counts_by_type.get(pt, {}).get(mid, 0)
          # ...
...
def list_materials_and_assembles():
    ...
    # 顯示字串（索引對應 num）
    str2 = ['未備料', '備料中', '備料完成', '等待組裝作業', '組裝進行中',
            '00/00/00', '檢驗進行中', '00/00/00', '雷射進行中', '00/00/00',
            '等待入庫作業', '入庫進行中', '入庫完成']

    # 取出全部 Material（可考慮加條件、分頁）
    materials = s.query(Material).all()

    # 1) 為每個 material 計算其「最大的 step_code」（供是否啟用之用）
    max_step_code_per_order = {}
    for m in materials:
        for a in m._assemble:
            step_code = a.process_step_code
            order_num_id = m.id
            if order_num_id not in max_step_code_per_order:
                max_step_code_per_order[order_num_id] = step_code
            else:
    ...
    # 2) 批次計算「有人在開工」的數量（以 material 為單位）
    counts_map = _active_count_map_by_material(s, [m.id for m in materials], process_type=21)
    ...
    for m in materials:
        # 原本邏輯：isAssembleStationShow 為 True 就跳過
        if m.isAssembleStationShow:
            continue

        # 顯示字串（狀態行）先取出 material 層級的 num
        num = int(m.show2_ok)
        base_token = str2[num]

        # 針對此 material 之每一個 assemble 建列
        pre_step_code = 99
        sub_process_step_enable = False
        ...
        for a in m._assemble:
            ...
            # 缺料併單（你原邏輯）
            if m.isLackMaterial == 0 and m.is_copied_from_id and m.is_copied_from_id > 0:
                continue

            # 是否啟用（最大 step_code / whichStation）
            step_enable = _step_enable(m, a, max_step_code_per_order)

            # 你的「副流程啟用」規則（保留）
            if (not step_enable):
                if pre_step_code == 0 and a.process_step_code != 0:
                    pre_step_code = a.process_step_code
                    sub_process_step_enable = True
                else:
                    pre_step_code = a.process_step_code
                    continue
            ...
            # 組工序名稱
            code = a.work_num[1:]  # 移除第一個字元
            name = code_to_name.get(code, '')
            format_name = f"{a.work_num}({name})"

            # 依規則替換 '00/00/00' 的顯示
            temp_str = _format_show2_ok(base_token, num, a.completed_qty, a.total_ask_qty_end)
            ...
            results.append({
                ...
                'assemble_process': '' if (num > 2 and not (step_enable or sub_process_step_enable)) else temp_str,
                'assemble_process_num': num,

                'assemble_id': a.id,
                'req_qty': m.material_qty,                  # 需求數量
                'delivery_qty': m.delivery_qty,             # 備料數量
                'total_receive_qty': f"({a.total_ask_qty})",
                'total_receive_qty_num': a.total_ask_qty,

                'must_receive_qty': a.must_receive_qty,
                'receive_qty': a.must_receive_qty,          # 2025-09-15 原注記
                'must_receive_end_qty': a.must_receive_qty,

                'delivery_date': m.material_delivery_date,
                'comment': m.material_comment.strip(),
                'isTakeOk': m.isTakeOk,
                'whichStation': m.whichStation,

                'isAssembleStation1TakeOk': m.isAssembleStation1TakeOk,
                'isAssembleStation2TakeOk': m.isAssembleStation2TakeOk,
                'isAssembleStation3TakeOk': m.isAssembleStation3TakeOk,

                'currentStartTime': a.currentStartTime,
                'tooltipVisible': False,
                'input_disable': a.input_disable,

                'process_step_enable': step_enable or sub_process_step_enable,
                'process_step_code': a.process_step_code,
                'isLackMaterial': m.isLackMaterial,

                'Incoming1_Abnormal': (a.Incoming1_Abnormal == ''),
                'is_copied_from_id': a.is_copied_from_id,

                # ✅ 新增：這個 material 是否有人在開工（以 material 為單位）
                'active_user_count': counts_map.get(m.id, 0),
            })

    s.close()

    status_ok = len(results) > 0
    print("listMaterialsAndAssembles, 總數:", len(results))

    # 若需要排序可在這裡集中控制（例如 order_num、or whichStation…）
    # results.sort(key=lambda x: (x['order_num'], x['isTakeOk']))

    return jsonify({
        'status': status_ok,
        'materials_and_assembles': results
    })
