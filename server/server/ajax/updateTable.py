    ...
    try:
        ...
        if is_partial_end:
            if finished_work_num == 'B109':
                ...
                cumulative_done_qty = to_int(process_completed_total)
                ...
                original_required_qty = max(
                    to_int(
                        getattr(assemble_record, 'total_ask_qty', 0)
                    ),
                    to_int(
                    ...
                    to_int(
                        getattr(assemble_record, 'must_receive_qty', 0 )
                    ),
                    0
                )
                ...
                for r in b109_rows:
                    ...
                    # 保存累計完成數量
                    r.total_completed_qty = cumulative_done_qty

                    r.allOk_qty = cumulative_done_qty
        ...
        if (
            ...
            child_b110_rework = (s.query(Assemble)
                .filter(Assemble.material_id == material_id)
            ...
            # ========================================================
            # 情況 2：有勾選 B110，但尚未預先建立 child B110
            #
            # B109 異常返工完成後：
            #   a1-異常 qty=10 隱藏
            #   建立 b1/b2/b3-異常，各 qty=10
            #
            # 正常 b1/b2/b3 qty=62 不受影響。
            # ========================================================
            if has_checked_b110_steps():
                ...
                checked_b110_steps = [
                    step
                    for step in (steps.get("check") or [])
                    if step.get("checked")
                ...
                existing_schedule_ids = {
                    to_int(r.schedule_id)
                    for r in existing_children
                }

                for step in checked_b110_steps:
                    schedule_id = to_int(step.get("id"))
                    ...
                    child = Assemble(
                        material_id=assemble_record.material_id,
                        material_num=assemble_record.material_num,
                        material_comment=assemble_record.material_comment,

                        # 依目前系統顯示方式保留檢驗工序順序
                        seq_num=str(step.get("id")),
                        ...
                        user_id='',
                        writer_id=assemble_record.writer_id,
                        write_date=assemble_record.write_date,
                        ...
                        # child 指向 a1-異常
                        is_copied_from_id=assemble_record.id,
                ...
                return jsonify({
                    ...
                    "message": (
                        "B109 abnormal rework finished, "
                        "created B110 abnormal rework rows"
                    )
                }), 200
        ...
        if (
            ...
            # ------------------------------------------------------------
            process_completed_total = (s.query(
            ...
            final_total = to_int(process_completed_total)
            ...
            if final_total <= 0:
                final_total = max(
                    ...
                    + current_done_qty,
                    to_int(
                        getattr(assemble_record, 'total_completed_qty', 0)
                    )
                    + current_done_qty,
                    to_int(
                        getattr(assemble_record, 'allOk_qty', 0)
                    )
                    + current_done_qty,
                    current_done_qty,
                    0
        ...
        if finished_work_num == 'B110':
            current_batch_no = to_int(
                getattr(assemble_record, 'release_batch_no', 0)
            )
            ...
            if not b110_rows:
                ...
                return jsonify({
                    ...
                    "material_done": False,
                    "waiting_send": False,
                    "abnormal_rework": current_is_abnormal,
                    "message": (
                        "No B110 rows found in current group"
                    )
                }), 200
            #
            ...
            def get_b110_done_qty(row):
                return max(
                    to_int(
                        getattr(row, 'total_completed_qty', 0)
                    ),
                    to_int(
                        getattr(row, 'allOk_qty', 0)
                    ),
                    to_int(
                        getattr(row, 'completed_qty', 0)
                    ),
                    0
                )
            ...
            # end if done_copy_rows and old_waiting_rows:
            ...
            if not_finished:
                # --------------------------------------------------------
                # 目前這筆已完成，但同組其他 B110 尚未全部完成。
                #
                # 正常例：
                #   b1 完成 → b1 隱藏，等待 b2/b3。
                #
                # 異常例：
                #   b1-異常完成 → b1-異常隱藏，
                #   b2-異常/b3-異常繼續顯示。
                ...
                assemble_record.must_receive_end_qty = current_total_qty
                ...
                # --------------------------------------------------------
                # 同組尚未完成的工序繼續留在 Begin／End。
                # 不可把已完成列重新打開。
                # --------------------------------------------------------
                for r in not_finished:
                    if to_int(r.id) == to_int(
                        assemble_record.id
                    ):
                        continue
                ...
                return jsonify({
                    ...
                    "message": (
                        "B110 row finished; "
                        "waiting for other rows in same group"
                    )
                }), 200
            #
            ...
            send_completed = get_b110_done_qty(send_row)
...
def update_assemble_process_step():
    ...
    try:
        ...
        # ============================================================
        # 20260909
        # 已完成異常返工量（只供有效完成量 / 剩餘量判斷）
        #
        # 不把返工量直接寫回正常 root 的 total_completed_qty，
        # 避免 release_b109_to_b110_batch() 再次合併時 double count。
        # ============================================================
        def get_finished_rework_qty(root_assemble_id, work_num='B109'):

            root_assemble_id = to_int(
                root_assemble_id
            )

            if root_assemble_id <= 0:
                return 0
            ...
            for r in assemble_records:
                ...
                if (
                    (getattr(r, 'reason', '') or '').strip()
                    != '異常返工'
                ):
                    continue

                if (
                    (getattr(r, 'work_num', '') or '').strip()
                    != work_num
                ):
                    continue
        ...
        # 判斷是否已達原始工序總量。
        #
        # 返工 row 自己仍使用原本 done_qty < must_qty。
        # ============================================================
        is_normal_b109 = (
            finished_work_num == 'B109'
            and
        ...
        effective_b109_done_qty = 0
        finished_rework_qty = 0
        original_b109_required_qty = 0

        if is_normal_b109:

            normal_process_completed_total = (
                s.query(
                    func.coalesce(
                        func.sum(
            ...
            normal_process_completed_total = to_int(
                normal_process_completed_total
            )

            finished_rework_qty = (
                get_finished_rework_qty(
                    assemble_record.id,
                    work_num='B109'
                )
            )

            original_b109_required_qty = max(
                to_int(
                    getattr(
                        assemble_record,
                        'total_ask_qty',
            ...
            if original_b109_required_qty <= 0:
                original_b109_required_qty = max(
                    to_int(must_qty),
                    normal_process_completed_total
                    + finished_rework_qty,
            ...
            effective_b109_done_qty = min(
                normal_process_completed_total
                + finished_rework_qty,
                original_b109_required_qty
            )

            is_partial_end = (
                effective_b109_done_qty
                < original_b109_required_qty
            )

            print(
                "[B109 EFFECTIVE END CHECK]",
                {
                    "material_id":
                        material_id,
        ...
        if is_partial_end:
            if finished_work_num == 'B109':
                ...
                # 正常 B109 自己的 Process 累計。
                # DB 的 total_completed_qty / allOk_qty 仍保存這個值，
                # 不直接包含異常返工，避免後續 release helper double count。
                normal_cumulative_done_qty = to_int(
                    process_completed_total
                )
                ...
                if original_required_qty <= 0:
                    original_required_qty = max(
                        to_int(must_qty),
                        normal_cumulative_done_qty
                        + finished_rework_qty,
                        0
                    )

                effective_cumulative_done_qty = min(
                    normal_cumulative_done_qty
                    + finished_rework_qty,
                    original_required_qty
                )
                ...
                release_result = {
                    ...
                    "release_qty": 0,
                    "created_ids": [],
                    "min_done_qty": effective_cumulative_done_qty,
                    "released_total": 0,
                    "message": (
                        "B109 partial finished; "
                        "B110 remains hidden until all B109 steps finish"
                ...
                return jsonify({
                    ...
                    "created_ids": release_result.get("created_ids", []),
                    "current_done_qty": current_done_qty,
                    "total_completed_qty": normal_cumulative_done_qty,
                    "effective_completed_qty": effective_cumulative_done_qty,
                    "finished_rework_qty": finished_rework_qty,
                    "remain_qty": remain_qty,
...
def update_assemble_process_step():
    ...
    try:
        ...
        if (
            ...
            #
            # 對 1863 而言：
            #   direct parent = 1848(B109)
            #
            # 但真正來源仍然是：
            #   1842(B110)
            #
            # 所以必須沿 is_copied_from_id 一直往上追，
            # 找到最近的一筆 B110 ancestor。
            # ========================================================

            direct_parent_row = None
            source_b110_row = None
            ...
            for _depth in range(30):
                ...
                if current_parent_id in visited_ids:
                    print(
                        "[B109 REWORK SOURCE LOOP DETECTED]",
                        {
                            "rework_id": assemble_record.id,
                            "loop_id": current_parent_id,
                        }
                    )
                    break
                ...
                # 記住直接 parent，debug 用
                if direct_parent_row is None:
                    direct_parent_row = current_parent

                current_work_num = (
                    current_parent.work_num or ''
                ).strip()

                # ----------------------------------------------------
                # 找到 B110：
                # 代表這條 B109 返工鏈最初是由 B110 異常產生
                # ----------------------------------------------------
                if current_work_num == 'B110':
                    source_b110_row = current_parent
                    break

                # ----------------------------------------------------
                # 還是 B109 / 其他衍生 row：
                # 繼續沿 is_copied_from_id 往上追
                # ----------------------------------------------------
                current_parent_id = to_int(
                    getattr(
                        current_parent,
                        'is_copied_from_id',
                        0
            ...
            # 原本變數名稱保留，降低後面程式修改範圍
            # --------------------------------------------------------
            parent_row = direct_parent_row

            parent_work_num = (
                (direct_parent_row.work_num or '').strip()
                if direct_parent_row
                else ''
            )

            is_from_b110_abnormal = (
                source_b110_row is not None
            )
            ...
            print(
                "[B109 REWORK SOURCE]",
                {
                    ...
                    "direct_parent_id":
                        (
                            direct_parent_row.id
                            if direct_parent_row
                            else None
                        ),

                    "direct_parent_work_num":
                        parent_work_num,
        ...
        if (
            ...
            # ========================================================
            # 20260909
            # 同 order_num 其他 material 是否仍有未完成 B109
            #
            # 重要：
            # 同一張訂單的工序可能被拆在不同 material / copy material。
            #
            # 例如 999900001886：
            #   material 633 -> 合爪+量爪
            #   material 648 -> 自動組立
            #   material 663 -> 自動鎖緊
            #
            # 所以不能只看目前 material_id 的 B109 是否完成。
            # 只要同 order_num 還有其他有效 B109 step > 0，
            # 目前工序完成後就必須隱藏，不可直接建立 waiting_send。
            # ========================================================
            ...
            if current_order_num:
                pending_order_b109_rows = (
                    ...
                    .join(
                        Material,
                        Material.id == Assemble.material_id
                    )
                    .filter(
                        Material.order_num == current_order_num
                    )
                    .filter(
                    ...
                    .filter(
                        Assemble.id != assemble_record.id
                    )
                    .filter(
                        or_(
                            Assemble.reason.is_(None),
            ...
            all_order_b109_done = (
                all_b109_done
                and len(pending_order_b109_rows) == 0
            )

            print(
                "[B109 ORDER FULL END CHECK]",
                {
                    "order_num": current_order_num,
                    "material_id": material_id,
                    "assemble_id": assemble_record.id,
                    "all_b109_done_current_material": all_b109_done,
                    "pending_order_b109": [
                        {
                            "id": row.id,
                            ...
                            "reason": row.reason,
                            "step": row.process_step_code,
                        }
                        for row in pending_order_b109_rows
                    ],
                    "all_order_b109_done": all_order_b109_done,
                }
            )
            ...
            if has_checked_b110_steps():
                ...
                # 20260909：
                # 只有「整張 order_num」所有 B109 都完成，
                # 才允許 B109-only 流程直接建立 waiting_send。
                if all_order_b109_done:
                    release_qty = current_done_qty
                    ...
                    # ----------------------------------------------------
                    # 尚未完成整張 order_num 的 B109。
                    #
                    # 若是「同一 material 內」的舊分批流程，
                    # 且目前 material 本身仍有 B109 未完成，
                    # 才保留原本 previous_total 的批次待送出行為。
                    #
                    # 若目前 material 已完成、只是「同 order_num 的其他
                    # material」還有後續 B109，則絕對不可建立 waiting_send。
                    # ----------------------------------------------------
                    ...
                    if not all_b109_done:
                        ...
                        if release_qty > 0:
                            ...
                            done_copy_row = create_b109_done_copy(
                                current_done_qty
                            )
                    else:
                        release_qty = 0
        ...
        if finished_work_num == 'B110':
            ...
            # ============================================================
            # 20260910
            # B110 異常返工完成後，先判斷是否還有下一個 B110 工序
            #
            # 例如：
            #
            #   B110 檢驗-異常 5
            ...
            # 不可直接 waiting_send。
            #
            # 只有目前 B110 已經是最後一道有效工序時，
            # 才繼續執行下面原本的 B110 waiting_send 邏輯。
            # ============================================================
            if current_is_abnormal:
                ...
                # --------------------------------------------------------
                # 取得實際有勾選、沒有刪除的 B110 工序。
                #
                # 注意：
                # 不使用 schedule_id 大小排序，
                # 直接保留 process_steps["check"] 原本順序，
                # 因為這才是真正的工序順序。
                # --------------------------------------------------------
                checked_b110_steps = [
                    step
                ...
                for idx, step in enumerate(checked_b110_steps):
                    if to_int(step.get("id")) == current_schedule_id:
                ...
                if (
                    ...
                    and
                    current_step_index + 1 < len(checked_b110_steps)
                ):
                    next_step = checked_b110_steps[
                        current_step_index + 1
                    ]

                print(
                    "[B110 ABNORMAL NEXT STEP CHECK]",
                    {
                        "material_id":
                            material_id,
                ...
                if next_step:
                    ...
                    assemble_record.completed_qty = next_qty
                    assemble_record.total_completed_qty = next_qty
                    assemble_record.allOk_qty = next_qty

                    assemble_record.must_receive_qty = next_qty
                    assemble_record.ask_qty = next_qty
                    assemble_record.total_ask_qty = next_qty
                    assemble_record.must_receive_end_qty = next_qty
                    ...
                    # 防鏽-異常
                    #
                    # child 必須指向目前這筆 assemble_record。
                    # ----------------------------------------------------
                    next_b110 = (
                        s.query(Assemble)
                        .filter(
                    ...
                    if next_b110:
                        ...
                        next_b110.process_step_code = 2

                        next_b110.must_receive_qty = next_qty
                        next_b110.ask_qty = next_qty
                        next_b110.total_ask_qty = next_qty
                        next_b110.total_ask_qty_end = 0
                        next_b110.must_receive_end_qty = next_qty

                        next_b110.abnormal_qty = 0

                        next_b110.completed_qty = 0
                        next_b110.total_completed_qty = 0
                        next_b110.allOk_qty = 0

                        next_b110.isAssembleStationShow = True
                        next_b110.isWarehouseStationShow = False

                        next_b110.input_disable = False
                        next_b110.input_end_disable = False
                        next_b110.input_abnormal_disable = False
                        next_b110.input_allOk_disable = True

                        next_b110.currentStartTime = None
                        next_b110.currentEndTime = None

                        next_b110.show1_ok = 1
                        next_b110.show2_ok = 5
                        next_b110.show3_ok = 5
                    ...
                    # ====================================================
                        ...
                        next_b110 = Assemble(
                            ...
                            must_receive_qty=next_qty,
                            ask_qty=next_qty,
                            total_ask_qty=next_qty,
                            total_ask_qty_end=0,
                            must_receive_end_qty=next_qty,
                    ...
                    print(
                        "[B110 ABNORMAL RELEASE NEXT STEP]",
                        {
                            "current_id":
                                assemble_record.id,
                    ...
                    return jsonify({
                        ...
                        "message":
                            "B110 abnormal finished and released to next B110 step",
                    }), 200
...
def send_assemble_to_warehouse():
    ...
    try:
        ...
        # ============================================================
        # 20260911
        # Warehouse 完成量改為 logical order level 計算
        #
        # 缺料 / copy material，例如：
        #
        #   636 -> 651 -> 662
        #
        # 這些 material 是同一張成品訂單的流程 lineage，
        # 不可只依目前 material_id 計算 Warehouse 完成量；
        # 也不可把各 copy material 直接 SUM，否則會重複計算。
        #
        # 規則：
        # 1. normal：每個 material 取最大的有效完成量，copy material 再取 max
        # 2. abnormal rework：只計真正 leaf，已有 child 的 parent 不再計
        # 3. normal + abnormal leaf 後，cap 到原訂單需求量
        # 4. 同 order_num 全部 material 同步 assemble_qty / total_assemble_qty
        # ============================================================
        ...
        # ------------------------------------------------------------
        # 7-1. 取得同 order_num 的全部組裝線 material
        # ------------------------------------------------------------
        order_material_rows = (
            s.query(Material)
            .filter(
        ...
        order_material_ids = [
            safe_int(current_material.id)
            for current_material
            in order_material_rows
            if safe_int(current_material.id) > 0
        ]

        # ------------------------------------------------------------
        # 7-2. 原訂單需求量
        #
        # copy material 只是 lineage，不可 SUM material_qty。
        # ------------------------------------------------------------
        order_required_qty = max(
            [
                safe_int(
                    current_material.material_qty
                )
                for current_material
                in order_material_rows
            ]
        ...
        # ------------------------------------------------------------
        # 7-4. 取得整張 logical order 已送到 Warehouse 的完成列
        # ------------------------------------------------------------
        warehouse_rows = []
        ...
        # ------------------------------------------------------------
        # 7-5. normal 與 abnormal leaf 分開統計
        # ------------------------------------------------------------
        normal_qty_by_material = {}
        abnormal_leaf_qty = 0
        warehouse_debug_rows = []

        for current_row in warehouse_rows:
            current_id = safe_int(
                current_row.id
            )
            ...
            current_qty = max(
                safe_int(
                    current_row.completed_qty
                ),
                safe_int(
                    current_row.total_completed_qty
                ),
                safe_int(
                    current_row.allOk_qty
                ),
                0,
            )
            ...
            # --------------------------------------------------------
            # abnormal rework：只計 leaf
            # --------------------------------------------------------
            if current_reason == '異常返工':
                if current_id in abnormal_parent_ids:
                    warehouse_debug_rows.append({
                        "assemble_id": current_id,
                        "material_id": current_material_id,
                        "qty": current_qty,
                        "counted": False,
                        "reason": "abnormal_non_leaf",
                    })
                    continue

                abnormal_leaf_qty += current_qty
                ...
                warehouse_debug_rows.append({
                    ...
                    "qty": current_qty,
                    "counted": True,
                    "reason": "abnormal_leaf",
                })

                continue

            # --------------------------------------------------------
            # normal：同 material 不 SUM，取最大有效量
            # --------------------------------------------------------
            normal_qty_by_material[
                current_material_id
            ] = max(
        ...
        # ------------------------------------------------------------
        # 7-6. copy material 之間 normal 不相加，只取最大值
        # ------------------------------------------------------------
        ...
        print(
            ...
            "[20260911]",
            {
                ...
                "normal_warehouse_qty": normal_warehouse_qty,
                "abnormal_leaf_qty": abnormal_leaf_qty,
                "order_warehouse_qty_raw": order_warehouse_qty_raw,
                "order_warehouse_qty": order_warehouse_qty,
                "rows": warehouse_debug_rows,
            }
        )

        # ------------------------------------------------------------
        # 7-8. 同 logical order 的 material 同步 Warehouse 完成量
        #
        # 只同步數量欄位。
        # show2_ok / show3_ok / whichStation 等流程狀態，
        # 仍維持各 material 自己的實際狀態。
        # ------------------------------------------------------------
        for current_material in order_material_rows:
            current_material.assemble_qty = (
                order_warehouse_qty
            )

            current_material.total_assemble_qty = (
                order_warehouse_qty
            )

            # 應入庫總量不可因部分送出而縮小
            current_material.must_allOk_qty = max(
                safe_int(
                    current_material.must_allOk_qty
                ),
                safe_int(
                    current_material.delivery_qty
                ),
                safe_int(
                    current_material.material_qty
                ),
