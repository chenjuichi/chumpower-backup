...
class Program
{
	...
	{
		TcpListener server = new TcpListener(IPAddress.Any, 6600);
		server.Start();
		...
		PrintManualInstruction();     // 第一次印出提示

		_ = ProcessMessagesAsync();   // 開啟獨立 Task 處理訊息佇列
		//_ = ListenForKeyPressAsync(); // 鍵盤輸入監聽功能
		_ = ListenForKeyPress();    	// 背景監聽 L 鍵
		...
		while (true)
		{
			...
			PrintManualInstruction(); 			// Client 連線時再次印出提示
    ...
    // 特例處理 station1_loading_ready，加上 message=1
    //if (message == "station1_loading_ready")
    //{
    //    message = $"{{\"event\": \"{message}\", \"message\": 1}}";
    //}
	...
	static async Task ListenForKeyPressAsync()
	{
		Console.WriteLine("🔘 請按下 'L' 鍵來觸發 station1_loading_ready");

		while (true)
		{
			var key = Console.ReadKey(true); // true 表示不顯示按鍵
			if (key.Key == ConsoleKey.L)
			{
				...
				string fullMessage = $"station1_loading_ready:{jsonData}";
				Console.WriteLine($"🔘 按下 L 鍵，發送訊息：{fullMessage}");
				await SendMessageSequenceAsync(fullMessage, 0);
			}
	...
	static async Task ListenForKeyPress()
	{
		await Task.Run(() =>
		{
				while (true)
				{
						...
						if (key.Key == ConsoleKey.L)
						{
							Console.WriteLine("🔔 手動觸發 station1_loading_ready");
							_ = SendMessageSequenceAsync("station1_loading_ready", 1000);
						}
				}
		});
	}
	...
	static void PrintManualInstruction()
	{
		Console.WriteLine("🔘 請按下 'L' 鍵來觸發 station1_loading_ready");
	}
