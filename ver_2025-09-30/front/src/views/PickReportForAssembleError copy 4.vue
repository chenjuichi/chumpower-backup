...
<div :class="['page_contain', { 'no-footer': !showFooter }]" :style="containerStyle" :key="componentKey">
  ...
  >
    <template v-slot:top>
      <v-card>
        <v-card-title class="d-flex align-center pe-2" style="font-weight:700; min-height:70px; height:70px;">
          <v-row>
            <v-col cols="12" md="3">
              組裝區異常填報
            </v-col>
    ...
    <template v-slot:item.cause_message="{ item }">
      <v-combobox
        ...
        class="combobox-no-wrap overflow-x-auto"
        chips
        multiple
        ref="comboboxRef"

        @update:model-value="moveWin"
      >
        <template v-slot:selection="{ attrs, item }">
          <v-chip v-bind="attrs" color="primary" variant="outlined">
...
const headers = [
  { title: '訂單編號', sortable: true, key: 'order_num', width:80 },
  { title: '現況進度', sortable: false, key: 'show1_ok', width:80 },
  { title: '現況備註', sortable: false, key: 'show3_ok', width:80 },
  { title: '交期', sortable: false, key: 'delivery_date', width:60 },
  { title: '訂單數量', sortable: false, key: 'req_qty', width:40 },
  { title: '現況數量', sortable: false, key: 'delivery_qty', width:40 },
  { title: '點檢人員', sortable: false, key: 'user', width:90 },
  { title: '說明', align: 'start', sortable: false, key: 'comment', width:180 },
  { title: '異常原因填寫', sortable: false, key: 'cause_message', width:180 },
...
:deep(div.v-combobox__selection) {
  //position: relative;
  //top: -10px;

  max-width: 60px;          // 限制 chip 的最大寬度
  overflow: hidden;         // 隱藏溢出的文字
  text-overflow: ellipsis; // 顯示省略號
  white-space: nowrap;      // 防止文字換行
}
/*
:deep(.combobox-with-scroll .v-field__input) {
  display: flex !important;
  flex-wrap: nowrap !important;  // 讓 v-chip 水平排列
  overflow-x: auto !important;   // 啟用水平滾動
  white-space: nowrap;
  max-width: 100%;
}

:deep(.combobox-with-scroll .v-chip) {
  flex-shrink: 0;   // 防止 v-chip 被壓縮
}
*/
...
:deep(.combobox-with-scroll .v-field__input) {
  ...
  display: flex;         // 讓 v-chip 在同一行排列
  flex-wrap: nowrap;     // 防止 chips 換行
...
// 確保 combobox 內部 chips 不會影響 table
/*
:deep(.combobox-with-scroll) {
  display: flex;
  align-items: center;
  justify-content: flex-start;
  max-width: 100%;
  overflow: hidden; // 避免影響 table 的 layout
}

// 讓 v-field__input 內部 chips 保持單行，並可滾動
:deep(.combobox-with-scroll .v-field__input) {
  display: flex;
  flex-wrap: nowrap;  // 不讓 chips 換行
  overflow-x: auto;   // 啟用水平滾動條
  white-space: nowrap;
  max-width: 100%;
}

// 限制 chip 容器的寬度，避免影響表格
:deep(.chip-container) {
  display: inline-flex;
  white-space: nowrap;
  max-width: 100%;
  overflow-x: auto;       // 讓單元格內部滾動，而不是影響整個表格
  scrollbar-width: thin;  // 確保滾動條較小，不影響 UI
}

// 自訂滾動條樣式，讓它更小
:deep(.combobox-with-scroll .v-field__input::-webkit-scrollbar) {
  height: 6px;
}

:deep(.combobox-with-scroll .v-field__input::-webkit-scrollbar-thumb) {
  background: #888;
  border-radius: 3px;
}
*/

/* 確保 combobox 內部 chips 水平滾動 */
:deep(.combobox-with-scroll .v-field__input) {
  display: flex;
...
:deep(.combobox-no-wrap .v-field__input) {
  display: flex;
...
/* 讓 chip 容器內部滾動，而不影響整個 table */
:deep(.chip-container) {
  display: inline-flex;
  overflow-x: auto; /* 內部水平滾動 */
  white-space: nowrap;
  max-width: 100%;
  //scrollbar-width: thin;
}
...
:deep(.custom-combobox .v-field__input::-webkit-scrollbar) {
  //height: 6px; // 調整滾動條高度
  background: #888;
  border-radius: 4px;
}

:deep(.custom-combobox .v-field__input::-webkit-scrollbar-thumb) {
  background: #888;
  border-radius: 3px;
}

:deep(.custom-combobox) {
  max-width: 300px; // 限制 combobox 寬度，讓滾動條與 combobox 結合
  overflow: hidden;
}
</style>
