	...
	<div>
		...
		>
			...
			<div v-if="selectedFile">
				...
				<!-- 條碼內容與插入按鈕 -->
				<div>
					<vue3-barcode :value="barcodeText" format="CODE128" height=25 width=1 :text="barcodeText" />
					<button @click="insertBarcode">插入條碼</button>
				</div>
				<!-- 保存更新內容 -->
...
const selectFile = (fileName) => {
	...
	//console.log("barcodeText:", barcodeText.value)
...
const saveFileFun = async () => {
	const payload = {
		filepath: `${currentPath.value}\\${selectedFile.value}`, // 拼接完整路徑
		updated_content: fileContent.value, // PDF 更新內容
		barcode_text: barcodeText.value, // 條碼文字
	};

	//console.log("saveFileFun, payload:", payload); // 調試輸出
	...
	try {
		...
		if (response instanceof Blob) {
			...
			// 不顯示視窗的方式：觸發下載
			link.style.display = 'none'; // 隱藏 download 按鈕
			document.body.appendChild(link);
			link.click();
			document.body.removeChild(link);
			window.URL.revokeObjectURL(url); // 清除 URL 以釋放內存
		} else {
