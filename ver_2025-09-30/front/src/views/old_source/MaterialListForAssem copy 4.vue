<template>
  <div :class="['page_contain', { 'no-footer': !showFooter }]" :style="containerStyle">
    ...
    >
      ...
      <template v-slot:header.data-table-select>
        <span class="custom-header">組裝區</span>
      </template>
        ...
        <v-card>
          <v-card-title class="d-flex align-center pe-2" style="font-weight:700;">
            ...
            <v-btn
              ...
              color="primary"
              variant="outlined"
              style="position: relative; right: 450px; top: 0px;"
          ...
          <!--
            <div class="button-container">
              <v-btn-toggle v-model="toggle_exclusive" class="group">
                <v-btn
                  variant="outlined"
                  :style="{ backgroundColor: toggle_exclusive === 1 ? '#e67e22' : '#fff', color: toggle_exclusive === 1 ? '#fff' : '#000' ,
                            position: 'relative', transition: 'background-color 0.3s ease, color 0.3s ease'}"
                  @click="setActive(1)"
                >
                ...
                <v-btn
                  variant="outlined"
                  :style="{ backgroundColor: toggle_exclusive === 2 ? '#27ae60' : '#fff', color: toggle_exclusive === 2 ? '#fff' : '#000' ,
                            position: 'relative', transition: 'background-color 0.3s ease, color 0.3s ease'}"
                  @click="setActive(2)"
                >
                  <span>呼叫AGV</span>
                ...
                <div class="bg" :style="{ transform: `translateX(${(toggle_exclusive - 1) * 100}%)` }"></div>
              </v-btn-toggle>
            </div>
          -->
            <div class="button-container">
              <v-btn-toggle v-model="toggle_exclusive" class="group">
                  <v-btn
                    variant="outlined"
                    :style="{
                      background: toggle_exclusive === 1 ? '#e67e22' : '#fff',
                  ...
                  <v-btn
                    variant="outlined"
                    :style="{
                      background: toggle_exclusive === 2 ? '#27ae60' : '#fff',
                ...
                <!--<div class="bg" :style="{ transform: `translateX(${(toggle_exclusive - 1) * 100}%)`, zIndex: '-1' }"></div>-->
                <!--
                <transition name="fade">
                  <div class="bg" v-if="toggle_exclusive === 1"></div>
                </transition>
                -->
              </v-btn-toggle>
            </div>
            ...
            <div class="flip_wrapper">
              <div class="flip_btn">
                <!-- 預設按鈕 -->
                <v-btn :disabled="isBlinking" color="primary" class="side default-side mt-n1 mr-15 mx-auto">
                  <v-icon left>mdi-account-arrow-right-outline</v-icon>備料送出</v-btn>
                <!-- 翻轉後的按鈕 -->
                <v-btn :disabled="isBlinking" color="primary" class="side hover-side mt-n1 mr-15 mx-auto" @click="callAGV">
                  <v-icon left size="24px">mdi-check-circle-outline</v-icon>確定</v-btn>
              </div>
            </div>
  ...
  import { apiOperation, setupGetBomsWatcher}  from '../mixins/crud.js';
    ...
    //=== data ===
    const toggle_exclusive = ref(1); // 控制選擇的按鈕
    ...
    { title: '訂單編號', sortable: true, key: 'order_num' },
      ...
      { title: '送料數量', sortable: false, key: 'receive_qty' },
  ...
  const handleKeyDown = (event) => {
    ...
    const isSelected = (item) => {
      console.log("item.columns.id", item.columns.id); // 查看 item.columns 是否包含 id
  ...
  .group {
    position: relative;     // 使背景可以相對於 group 定位
  }
  /*
  .bg {
    //background-color: #e67e22;    // 背景顏色初始值
    background: #e67e22;    // 背景顏色初始值
    //height: 100%;                     // 背景的高度與按鈕相同
    //width: 50%;                       // 設定為兩個按鈕的一半
    //position: absolute;
    //top: 0;
    //left: 0;
