<template>
      ...
      <v-card flat class="card-container">
        ...
        <!--
          <template v-slot:item.receive_qty="{ item }">
            ...
            <v-form ref="receiveQtyForm">
            <v-col class="mt-5" style="position: relative;">
              <v-text-field
                ...
                :rules="[() => (item.tooltipVisible ? '領取數量超過需求數量' : true)]"
              />
            </v-col>
            </v-form>
          </template>
        -->
                <template v-slot:item.receive_qty="{ item }">
                  ...
                  <span
                    ...
                    style="position: absolute; left: 45px; top: 0; z-index: 2; background-color: white; padding: 0; min-width: 180px; white-space: nowrap; color:red; text-align: left; font-weight: 700;"
                  >領取數量超過需求數量</span>
                  </div>
                </template>
  ...
  const checkReceiveQty = (item) => {
    ...
    if (total > temp) {
      ...
      item.tooltipVisible = false;  // 符合條件則隱藏 tooltip
    }
  };
  */
  const checkReceiveQty = (item) => {
    ...
    const total = Number(item.receive_qty) + Number(item.total_receive_qty_num);
    const temp = Number(item.req_qty)
    console.log("receive_qty, total_receive_qty", Number(item.receive_qty), Number(item.total_receive_qty_num));
    ...
    if (total > temp) {
      ...
      item.tooltipVisible = true;  // 顯示 Tooltip
      setTimeout(() => {
        item.tooltipVisible = false;  // 5 秒後隱藏 Tooltip
        //if (receiveQtyForm.value) {
        //  receiveQtyForm.value.validate();  // 手動觸發表單驗證
        //}
      }, 3000);
      console.error('領取數量超過需求數量');
    } else {
      console.log("total, temp, step2...");
      item.tooltipVisible = false;
      //if (receiveQtyForm.value) {
      //  receiveQtyForm.value.validate();  // 驗證通過時手動觸發驗證
      ////}
    }
  };
  /*
