...
const splitName = computed(() => {
	if (!currentUser.value) return { last_name: '', first_name: '' };
	// 處理4字姓名：假設是複姓(2字)+名字(2字)
	if (currentUser.value.length === 4) {
		return {
			last_name: currentUser.value.substring(0, 2),
			first_name: currentUser.value.slice(2, 4)
		};
	}
	// 處理3字姓名
	if (currentUser.value.length === 3) {
		return {
			last_name: currentUser.value[0],
			first_name: currentUser.value.substring(1, 3)
		};
	}
	// 默認處理2字姓名
	return {
		last_name: currentUser.value[0],
		first_name: currentUser.value[1] || '' // 防止單字姓名時 undefined
	};
});
