<template>
      ...
      <template v-slot:top>
        <v-card>
          <v-card-title class="d-flex align-center pe-2" style="font-weight:700;">
            ...
            <!--客製化 員工選單-->
            <div style="position: relative; right: 120px; width: 160px; height:80px; overflow: hidden;" v-show="showMenu">
              ...
              <v-text-field
                ...
                variant="outlined"
                density="comfortable"
                style="min-width: 160px; width: 160px; position: absolute; z-index: 2;"
              />
              ...
              <v-select
                ...
                variant="outlined"
                density="comfortable"
                @update:modelValue="updateTextFieldFromSelect"
                style="
                  min-width: 160px;
                  width: 160px;
                  position: relative;
  ...
  const placeholderText = ref('請選擇員工')
  const inputSelectValue = ref(null)
  ...
  //=== computed ===
  /*
  const normalizedDesserts = computed(() =>
    desserts.value.map(emp => ({
      ...emp,
      emp_id: emp.emp_id.replace(/^0+/, ''),      // 移除前導 0
      display: `${emp.emp_id}  ${emp.emp_name}`,  // 顯示 "員工編號  員工姓名"
    }))
  );

  const formattedDesserts = computed(() => normalizedDesserts.value);
    */
   /*
  // 格式化員工資料，供 v-select 使用
...
// 格式化的員工文本
const formattedEmployeeText = computed(() => {
  const selected = desserts.value.find(emp => emp.emp_id === selectedEmployee.value);
  return selected ? `${selected.emp_id} ${selected.emp_name}` : inputEmployeeId.value;
});
*/
/*
// 格式化員工資料，供 v-select 使用
...
// 格式化的員工文本，用於顯示在 v-text-field
const formattedEmployeeText = computed(() => {
  const selected = desserts.value.find(emp => emp.emp_id === selectedEmployee.value);
  return selected ? `${selected.emp_id} ${selected.emp_name}` : '';
});
*/
/*
// 格式化員工資料，供 v-select 使用
...
const formattedEmployeeText = computed(() => {
  const selected = desserts.value.find(emp => emp.emp_id === inputSelectValue.value);
  return selected ? `${selected.emp_id} ${selected.emp_name}` : '';
});
  ...
  const handleEmployeeSearch = () => {
    ...
    const selected = desserts.value.find(emp => emp.emp_id.replace(/^0+/, '') === selectedEmployee.value);
    if (selected) {
      selectedEmployee.value = `${selected.emp_id} ${selected.emp_name}`;
      console.log("已更新選中員工: ", selectedEmployee.value);

      inputSelectValue.value = placeholderText.value;
    } else {
      selectedEmployee.value = ''; // 清空值，防止未選擇時顯示錯誤內容
    }

      // 確保 placeholder 保持靜態文字
      placeholderText.value = "請選擇員工";


    /*
    const selected = desserts.value.find(emp => emp.emp_id === inputSelectValue.value);

    const found = desserts.value.find(emp => emp.emp_id === inputEmployeeId.value.trim());
    if (found) {
      selectedEmployee.value = found.emp_id;
      inputEmployeeId.value = `${found.emp_id} ${found.emp_name}`;
    //} else {
    //  alert('未找到對應員工');
    }
    */
  ...
  const updateTextFieldFromSelect = () => {
    console.log("更新 TextField: ", inputSelectValue.value);
    const selected = desserts.value.find(emp => emp.emp_id === inputSelectValue.value);
    if (selected) {
      selectedEmployee.value = `${selected.emp_id} ${selected.emp_name}`;
      console.log("已更新選中員工: ", selectedEmployee.value);
