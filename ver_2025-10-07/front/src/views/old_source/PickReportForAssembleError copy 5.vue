...
watch(() => informations_for_assemble_error.value || [], (newVal) => {
    console.log("Updated cause_message:", newVal);
  },
  { deep: true }
);

watch(fromDateValStart, (newValue) => {
    if (newValue) {
      console.log("新選擇的開始日期:", newValue);
    }
  }
);

watch(fromDateValEnd, (newValue) => {
    if (newValue) {
      console.log("新選擇的截止日期:", newValue);
    }
  }
);
...
const compareDateStart = computed(() => {
  if (!fromDateValStart.value) return ""; // 確保不為空
  const dateObj = new Date(fromDateValStart.value); // 轉成 Date 物件
  return convertToMinguoDate(dateObj);
});

const compareDateEnd = computed(() => {
  if (!fromDateValEnd.value) return "";
  const dateObj = new Date(fromDateValEnd.value);
  return convertToMinguoDate(dateObj);
});
//=== mounted ===
onMounted(async () => {
...
const convertToMinguoDate = (date) => {
  console.log("convertToMinguoDate(), date:", date)
  ...
  let yy_value = date.getFullYear(); // 獲取西元年
  let mm_value = String(date.getMonth() + 1).padStart(2, "0"); // 補零的月份
  let dd_value = String(date.getDate()).padStart(2, "0"); // 補零的日期

  return `${yy_value}-${mm_value}-${dd_value}`; // 回傳 yyyy-mm-dd
};
...
const convertToMinguoDate = (date) => {
  ...
  let yy_value = date.getFullYear(); // 獲取西元年
  let mm_value = String(date.getMonth() + 1).padStart(2, "0");  // 獲取月份 (補 0)
  let dd_value = String(date.getDate()).padStart(2, "0");       // 獲取日期 (補 0)

  let b = yy_value - 1911; // 轉換為民國年
  return `${b}/${mm_value}/${dd_value}`; // 格式化輸出
};
*/
...
// 調整標題字體
:deep(.date-picker-scale .v-date-picker-header) {
  font-size: 14px !important;
}

// 縮小格子間距
:deep(.date-picker-scale .v-date-picker-table) {
  padding: 4px !important;
}

// 日期格子的寬度
:deep(.date-picker-scale .v-date-picker-table td) {
