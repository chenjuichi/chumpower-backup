...
def to_aware_dt(val):
    """把 DB 取出的值（datetime 或 str 或 None）轉成 UTC-aware datetime；失敗回 None。"""
    if not val:
        return None
...
def start_process():
    ...
    # ---- 動態計算 live elapsed（秒） ----
    live_elapsed = int(log.elapsedActive_time or 0)

    # 用 UTC；你也可用本地時區，但要一致
    now = datetime.now(timezone.utc)

    # begin_time 可能是 naive，轉成有時區的時間以避免報錯
    #begin = log.begin_time
    begin  = to_aware_dt(getattr(log, 'begin_time', None))
    pstart = to_aware_dt(getattr(log, 'pause_started_at', None))

    is_paused = bool(getattr(log, 'is_pause', True))
    pause_sum = int(getattr(log, 'pause_time', 0) or 0)

    if not is_paused and begin:
        # 若此刻正處於一段未結束的暫停，補進總暫停秒
        if pstart and pstart <= now:
            pause_sum += max(0, int((now - pstart).total_seconds()))
        live_elapsed = max(0, int((now - begin).total_seconds()) - pause_sum)
    '''
    if begin is not None and begin.tzinfo is None:
        begin = begin.replace(tzinfo=timezone.utc)

    if not log.is_pause and begin:
        pause_total = int(log.pause_time or 0)

        # 若此刻正處於暫停區間（還沒恢復），把這段也加進 pause_total
        #（通常 start API 不會遇到「正暫停」但保險起見）
        if getattr(log, "pause_started_at", None):
            ps = log.pause_started_at
            if ps is not None and ps.tzinfo is None:
                ps = ps.replace(tzinfo=timezone.utc)
            if ps:
                pause_total += max(0, int((now - ps).total_seconds()))

        live_elapsed = max(0, int((now - begin).total_seconds()) - pause_total)
    '''
    return jsonify({
        "success": True,
        "process_id": log.id,
        #"begin_time": log.begin_time,
        "begin_time": getattr(log, 'begin_time', None),
        #"elapsed_time": log.elapsedActive_time or 0,
        "elapsed_time": int(live_elapsed),
        #"is_paused": bool(log.is_pause),
        "is_paused": is_paused,
        #"pause_time": int(log.pause_time or 0),
        "pause_time": pause_sum,
        #"pause_count": int(log.pause_count or 0),
        #"pause_count": int(getattr(log, 'pause_count', 0) or 0),  # add
        #"has_started": bool(log.has_started),
        "has_started": bool(getattr(log, 'has_started', False)),  # modify

        #"isOpen": material_record.isOpen,
        #"hasStarted": material_record.hasStarted,
        #"startStatus": material_record.startStatus,
        #"startStatus": not bool(log.is_pause),          # modify
        #"isOpenEmpId": material_record.isOpenEmpId,

        "isOpen": bool(getattr(material_record, 'isOpen', False)) if material_record else False,            # modify
        "hasStarted": bool(getattr(material_record, 'hasStarted', False)) if material_record else False,    # modify
        "startStatus": bool(getattr(material_record, 'startStatus', False)) if material_record else False,  # modify
        "isOpenEmpId": getattr(material_record, 'isOpenEmpId', None) if material_record else None,    # modify
...
def toggle_process():
    """
    切換暫停/恢復：
      - is_paused=True  → 進入暫停：只記下 pause_started_at（若當前不是暫停）
    ...
    # ---- 統一用台北 aware datetime ----
    now_aw = datetime.now(TPE).replace(microsecond=0)

    # 小工具：把 DB 取出的值轉成 aware datetime
    def to_aw(dt):
        if not dt:
            return None
        ...
        if isinstance(dt, str):
            # 盡量吃常見格式 / ISO 8601
            try:
                return datetime.fromisoformat(dt.replace('Z', '+00:00')).astimezone(TPE)
            except Exception:
                pass
            for fmt in ("%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S"):
                try:
                    return datetime.strptime(dt, fmt).replace(tzinfo=TPE)
                except Exception:
                    continue
        return None

    # 轉既有欄位
    begin_aw = to_aw(getattr(log, "begin_time", None))
    ps_aw    = to_aw(getattr(log, "pause_started_at", None))
    pause_sum = int(getattr(log, "pause_time", 0) or 0)
    is_paused = bool(getattr(log, "is_pause", True))
    ...
    if want_pause:
        ...
        if not is_paused:
            # 從「跑」→「暫停」：開始一段暫停
            log.is_pause = True
            log.pause_started_at = now_aw  # 存 aware datetime
            is_paused = True
            ps_aw = log.pause_started_at
        # 若本來就暫停，什麼都不做
    else:
        # → 要恢復
        if is_paused:
            # 從「暫停」→「恢復」：把這段暫停累加
            if ps_aw:
                delta = max(0, int((now_aw - ps_aw).total_seconds()))
                pause_sum = (pause_sum or 0) + delta
                log.pause_time = pause_sum
            log.pause_started_at = None
            log.is_pause = False
            is_paused = False

        # 第一次按開始：補 begin_time + has_started
        if not bool(getattr(log, "has_started", False)):
            log.has_started = True
            if not begin_aw:
                log.begin_time = now_aw
                begin_aw = log.begin_time

    s.commit()

    # ---- 回傳「即時」累積秒（避免前端看到 0）----
    live_elapsed = int(getattr(log, "elapsedActive_time", 0) or 0)
    if not is_paused and begin_aw:
        # 若此刻正處於未結束的暫停（理論上不會，但保險）
        ps_aw = to_aw(getattr(log, "pause_started_at", None))
        tmp_pause = pause_sum
        if ps_aw and ps_aw <= now_aw:
            tmp_pause += max(0, int((now_aw - ps_aw).total_seconds()))
        live_elapsed = max(0, int((now_aw - begin_aw).total_seconds()) - tmp_pause)

    return jsonify(
        success=True,
        is_paused=is_paused,
        elapsed_time=int(live_elapsed),  # ★ 即時秒數
        pause_time=int(pause_sum),
        ...
        begin_time=log.begin_time.isoformat() if log.begin_time else None,
    )
'''
    ...
    if want_pause:
        ...
        if not getattr(log, "has_started", False):
            ...
            if not log.begin_time:
                ...
                log.begin_time = datetime.now(TPE).replace(microsecond=0)   # ★ 存 datetime，不是字串
